...
class Meteor(pg.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pg.image.load("source/img/meteor_med.png").convert_alpha()
        self.rect = self.image.get_rect()
...
all_sprites.add(myMeteor)
meteor_sprites.add(myMeteor)
...
while True:
    ...
    hits = pg.sprite.spritecollide(myShip,meteor_sprites,True)
    if len(hits)>0:
        hits[0].rect.x += 5 
